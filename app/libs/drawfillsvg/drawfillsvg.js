...
(function( window ){
	...
	*/
	...
	DrawFillSVG.prototype._init = function() {
		...
		this.filldelay = this.options.filldelay;
		this.stager = this.options.stager;
	...
	*/
	...
	DrawFillSVG.prototype._initAnimation = function() {
		...
		path.style.transition = path.style.WebkitTransition = "stroke-dashoffset " + delay + "s ease-in-out";
		// go baby go
		path.style.strokeDashoffset = 0;
		// fill the path
		th._fillPath(path);
		if(th.paths.length>1)
		{
			var timerId = setInterval(function() {
				var path = th.paths[i + 1];
				var length = path.getTotalLength();
				// reset opacities
				path.style.strokeOpacity = 1;
				// reset transitions
	...
	*/
	...
	DrawFillSVG.prototype._fillPath = function(path) {
		...
		path.addEventListener( transEndEventName, function() {
			// reset transitions
			path.style.transition = path.style.WebkitTransition = "none";
			path.style.transition = path.style.WebkitTransition = "fill-opacity " + th.filldelay + "s ease-in-out";
